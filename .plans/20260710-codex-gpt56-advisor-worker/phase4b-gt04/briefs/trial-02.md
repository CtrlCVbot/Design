[WORKER BRIEF]
brief_id: GT04-trial-02-01
role: routine_worker
runtime_agent: registered custom agent routine_worker
objective:
- Normalize the four JSON records in trial-02/records exactly as defined and make the deterministic grader pass.
why:
...
- repo state: disposable fixture GT04-json-normalization-v1; trial-02 is byte-identical to immutable baseline and currently has 4 grader mismatches
- current behavior: python -B verify_gt04.py --trial trial-02 exits 1 with mismatches 4
...
- relevant files: trial-02/records/*.json are the only writable inputs; expected/records and verify_gt04.py are read-only acceptance evidence
allowed_paths:
- C:\Work\Dev\Design\.plans\20260710-codex-gpt56-advisor-worker\phase4b-gt04\fixture\trial-02\records\*.json
forbidden_paths:
- baseline/**
- expected/**
- verify_gt04.py
- trial paths other than trial-02
...
- from fixture root run python -B verify_gt04.py --trial trial-02
...
required_tests:
- python -B verify_gt04.py --trial trial-02 -> expected: PASS, mismatches 0
parallel_ownership:
- worker owns only: fixture/trial-02/records/*.json
- shared files/state: trial-01 and trial-03 are owned by other routine Workers; baseline, expected, verifier, and all global state are read-only
stop_conditions:
...
acceptance_check:
- python -B verify_gt04.py --trial trial-02 returns exit 0 and JSON status PASS with mismatches 0
stop_if:
