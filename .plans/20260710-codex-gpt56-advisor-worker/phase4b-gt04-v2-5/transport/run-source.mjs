...
const expectedHash = 'DBB0A0270149B65CFDE64D01FA22419CA4B9A7D1133A5F819189EEA159CD7174';
const briefPath = 'C:/Work/Dev/Design/.plans/20260710-codex-gpt56-advisor-worker/phase4b-gt04-v2-5/briefs/trial-01.md';
...
function decodeShellOutput(value) {
  ...
  const textValue = typeof payload.text === 'string'
    ? payload.text
    : payload.text && typeof payload.text.value === 'string'
      ? payload.text.value
      : null;
  if (typeof textValue !== 'string' || typeof payload.sha256 !== 'string') throw new Error('Shell payload is invalid');
  if (textValue.indexOf(expectedHeader) < 0) throw new Error('Worker brief header is missing');
  if (payload.sha256 !== expectedHash) throw new Error('Worker brief hash is invalid');
  return { text: textValue, sha256: payload.sha256 };
}
