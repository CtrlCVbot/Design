...
1. 포트를 고정(예: 3002)하지 말고, **이 환경에 맞는 개발 서버를 새로 띄운다** (예: `npm run dev` / `pnpm dev`, 백그라운드 실행).
2. 서버가 **실제로 바인딩한 포트**를 기동 로그에서 확인한다 (`ready on http://localhost:XXXX`). 기본 포트가 이미 사용 중이면 자동으로 다음 포트로 뜨므로, 로그에 찍힌 포트를 그대로 사용한다.
3. 그 주소의 `/login`(`http://localhost:<확인한 포트>/login`)에 접속 → 테스트 계정 로그인 → 대상 페이지 이동
